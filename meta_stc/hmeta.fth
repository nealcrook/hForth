\ $Id: hmeta.fth,v 1.10 1998/11/25 21:48:57 crook Exp crook $
\ $Log: hmeta.fth,v $
...
\ Define the threading model. The default is direct threading code (DTC).
\ Option is to use subroutine threading code (STC).
\ TRUE hCONSTANT STC_MODEL
FALSE hCONSTANT STC_MODEL
...
\ TODO this gets redefined later, but we need it really early in hforth.fth..
: code, _CODE t! _CODE CELLL + TO _CODE ;

\ Note the distinction between the target words code, and COMPILE, -- the
\ former just spits out a value from the stack into code space. The latter
\ appends the execution semantics associated with the XT on the stack. For a
\ DTC forth, code, and COMPILE, have identical behaviour, but for an STC forth
\ that is NOT the case.
STC_MODEL [IF]
: tCOMPILE, ( xt -- ) \ emit a BL to get to the address specified by xt.
  	_CODE ALIGNED DUP TO _CODE
	- 8 - 2 RSHIFT    \ get signed offset
	00FFFFFF AND                 \ mask off high-order sign bits
	EB000000 OR            \ make the opcode 
	_CODE t! _CODE CELLL + TO _CODE ; \ emit it and purge the block
[ELSE]
: tCOMPILE, _CODE t! _CODE CELLL + TO _CODE ;
[THEN]

\ \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
\ Vectors for the assembler

\ Code generation - emit n at the target PC and increment the target PC
:NONAME ( n -- ) _CODE t! _CODE CELLL + TO _CODE ; 'ASM,32 !

\ Return the target PC - an absolute address in TARGET space
:NONAME ( -- n ) _CODE ; 'ASM. !

\ Read/Write addresses in target space
' t@ 'ASM@ !
' t! 'ASM! !
...
\ TRUE hCONSTANT META_HI
FALSE hCONSTANT META_HI

\ Force unproven definitions where available
TRUE hCONSTANT META_EXPERIMENTAL

\ Force code endings to branch through micro-debugger
FALSE hCONSTANT MICRO_DEBUG
